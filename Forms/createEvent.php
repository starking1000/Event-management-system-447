...
<body>
    ...
    if(isset($_REQUEST['event_name'])){
        ...
        $sql = "INSERT INTO events VALUES (null, '$event_name','$venue','$max_capacity','$time','$date')";
        $result = mysqli_query($conn, $sql);

        if ($result){
            echo " <div class='container'> SUCCESSFULLY SAVED ADDED THE EVENT'<br/>
            <a href='../Tables/AllRsvps.php'>View rsvps</a><br/>
            <a href=`./makePayment.php?event_name=$event_name`>Make Payment</a>
            </div>";
            Header(`Location:./makePayment.php?event_name=$event_name`);
        } else {
            echo " <div class='container'>Error saving data <br/>
            <a href='./CreateEvent.php'>Try again</a>
            </div>";
        }
    } else {
?>
 <div class="container">
        ...
        <h5 style="text-align:center; margin-top:30px;"> Create An event at <?php echo $_GET['venue_name'];?></h5>
        <form method="POST">
            <input type="text" name="event_name" id="event_name" placeholder="Event Name">
            <input type="text" name="venue" id="venue" value="<?php echo $_GET['venue_name']?>" disabled placeholder="<?php echo $_GET['venue_name']?>">
            <input type="number" name="max_capacity" id="max_capacity" placeholder="max capacity of the event">
